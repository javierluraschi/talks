---
title: "Cluster Computing Made Easy"
subtitle: "with Spark and R"
...
tweets <- produce_tweets("#cascadiarconf", producer, "tweets")
```
...
```{r}
wordcloud::wordcloud(collect(words)$word, collect(words)$total, scale = c(4, 0.5),
...
image_response <- RoogleVision::getGoogleVisionResponse("http://pbs.twimg.com/media/D8kH_MsVsAA7WLM.jpg", numResults = 10)
image_response
```

```{r}
knitr::include_graphics("http://pbs.twimg.com/media/D8kH_MsVsAA7WLM.jpg")
```
...
```{r}
wordcloud::wordcloud(tags$description, tags$total, scale = c(4, 0.5),
                     min.freq = 1, max.words = 50)
```
