...
```{bash}
mlflow run --entry-point train.R https://github.com/rstudio/mlflow-exampl
```
...
mlflow_save_model(
  crate(~ stats::predict(model, .x), model, column)
)
```
...
mlflow_rfunc_predict(
  "model",
  data = rbind(c(7, 0.27, 0.36, 20.7, 0.045, 45, 170, 1.001, 3, 0.45, 8.8))
)
```
```
           s0
...
<img src="images/mlflow-r-serve.png" style="border: solid 1px #DDD">
